...
To set up these models, we went through the 7 steps. Step 1 - 6 are thereby performed exclusively on the labelled training data which was further split into a training and test set (80/20), with the training used to find the best parameters and the test data reserved to evaluate the final model before making predictons on the unlabelled data. 
...
The models allow to specify various training arguments such as batch size, learning rate, training epochs, or weight decay. The combination of these parameters can signifcantly impact both the computational resources required to run the model as well as the performance. To achieve the best possible performance (i.e. accuracy) of the model we set up a separate hyper-optimization process using Optina. This helped us find a good combination of parameters (epochs, batch size, and learning rate) which we then continued to slightly adjust manually (particularly the weigth decay to correct for overfitting) until we were satisifed with the performance of the model. Ideally, we would apply cross-validation during this hyper-optimziation process. However, due to limited computational resources, this was not possible which makes the process slightly less robust.
...
- **Step 6: Evaluation:**
After each fold, the model is evaluated on the test set. Here we primarily use accuracy as the evaluation metric. After all five folds, we take the average of the statistics to obtain the final evaluation of the model on the training data. 
