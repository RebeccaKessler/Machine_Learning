...
- **Step 6: Evaluation:**
After each fold, the model is evalauted on the evaluation dataset. Here we primarily use accuracy as the evaluation metric. After the five folds, we calculate the final accuracy by taking the average over all folds.
- **Step 7: Optimization:**
To increase the performance (i.e. accuracy) of the model we set up a hyper-optimization process using Optina. This helped us find a good combination of parameters (epochs, batch size, and learning rate) which we then further adjusted manually until we were satisfied with the performance level of the model. Ideall,y we would also apply cross-validation in this step. However, due to limited computational resources, this was not possible.
- **Step 8: Prediction:**
