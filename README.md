...
We then went one step further and combined the two models. Hence, we frist fine-tuned the CamemBert and Flaubert model individually and let them each make their predictions on the unlabelled data. These prediction are in the form of probabilities for each of the six classes. This allows us to implement an average voting which takes the average over the probabilities of the two models and then outputs the label that as acheived the higest average probability in the two models. This provids the most robust performance of our models.
