...
- **Step 7: Optimization:**
To increase the performance (i.e. accuracy) of the model we set up a hyper-optimization process using Optina. This helped us find a good combination of parameters (epochs, batch size, and learning rate) which we then further adjusted manually until we were satisfied with the performance level of the model. Ideally, we would also apply cross-validation during the hyperoptimziation process. However, due to limited computational resources, this was not possible.
- **Step 8: Prediction:**
