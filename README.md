...
- **Step 6: Evaluation:**
After each fold, the model is evaluated on the evaluation dataset. Here we primarily use accuracy as the evaluation metric. Based on the result, we continued to manually adjust the various parameters until we were satisifed with the performance of the model. In the end, we calculate the final accuracy on the training data by taking the average over all folds. 
